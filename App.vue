<script>
	...
	export default {
		globalData: {
			//	serverUrl: "http://172.20.203.21:8099",		// 开发环境，不能使用localhost
		...
		methods: {
			...
			getUserInfo() {
				var myUserId = this.getUserInfoSession().id
			...
			getUserIsDisable() {
				if (!this.getUserInfoSession()) return
				var myUserId = this.getUserInfoSession().id
				var serverUrl = getApp().globalData.serverUrl;
				uni.request({
					method: "GET",
					...
					success(result) {
						...
						if (res.status == 200) {
							uni.setStorageSync("userInfo", JSON.stringify(res.data));
							var isDisable = res.data.usersStatus;
							if (isDisable === '1') {
								uni.showToast({
									title: "Your account has been disabled, please contact the administrator",
									icon: "none",
									duration: 3000
								});
								uni.navigateTo({
			...
			getUserIsPlay() {
				...
				const isPlay = JSON.parse(uni.getStorageSync("userInfo")).clientStatus;
				return isPlay
			},
			clearUserInfo() {
