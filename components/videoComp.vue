...
<template>
	<view style="flex: 1;">
		...
		<list :pagingEnabled="true" :show-scrollbar="false" @scroll="listScroll" @scrollend="scroll" :scrollable="true">
			...
			<cell :recycle="false" v-for="(item, index) in playerList" :key="index" :data-index="index" :id="index"
				...
				<video v-if="playerCur === index" ref="myVideo" id="myVideo" :src="item.url"
...
<script>
	...
	export default {
		...
		methods: {
			isAuto() {
				return getApp().getUserIsPlay() == 0 && this.playStatus ? true : false
			},
			...
			onplay: function(e) {
				...
				console.log(this.playerList[this.playerCur])
			},
